...
const stripeLivePrefix = `sk_${'live'}_`;
...
test('placeholder secrets: marker words are filtered', () => {
  assert.equal(isLikelyPlaceholderSecret(`${stripeLivePrefix}YOUR_API_KEY_HERE`), true);
  assert.equal(isLikelyPlaceholderSecret('AIzaSyDUMMYKEYplaceholder1234567890abc'), true);
  assert.equal(isLikelyPlaceholderSecret(`${stripeLivePrefix}examplekeyexamplekeyexample`), true);
...
test('placeholder secrets: low-entropy / repeated / sequential filler is filtered', () => {
  assert.equal(isLikelyPlaceholderSecret(`${stripeLivePrefix}${'0'.repeat(24)}`), true); // all zeros
  assert.equal(isLikelyPlaceholderSecret(`${stripeLivePrefix}${'a'.repeat(24)}`), true); // one char
  assert.equal(isLikelyPlaceholderSecret(`${stripeLivePrefix}0123456789abcdefghijklmn`), true); // ascending run
});

test('placeholder secrets: realistic random keys are NOT filtered', () => {
  assert.equal(isLikelyPlaceholderSecret(`${stripeLivePrefix}51H8xN2eZvKYlo2Cq7Bd4RtP9wMz`), false);
